...
1.  **Profundidade Hierárquica**: Desenvolver as BCs até o Nível 3. Por exemplo:
    *   Nível 1: Planejamento e Gestão da Produção
        *   Nível 2: Planejamento da Produção
            *   Nível 3: Planejamento Mestre de Produção (MPS)
2.  **Foco em Manufacturing 4.0**: As BCs devem refletir conceitos como automação, conectividade (IoT), análise de dados, inteligência artificial, sistemas ciberfísicos, manufatura aditiva, realidade aumentada/virtual, etc., aplicados à produção de motocicletas.
3.  **Indústria de Motos**: Adaptar as capacidades genéricas de manufatura para as especificidades da produção de motocicletas (ex: customização, variedade de modelos, montagem de componentes complexos, controle de qualidade específico).
4.  **Definição Detalhada**: Para cada BC identificada, forneça uma descrição clara e concisa de sua função e importância. Não economize detalhes.
5.  **Dependências e Relações**: Identifique e liste explicitamente as dependências entre as BCs (quais capacidades são pré-requisitos para outras) e as relações onde uma capacidade habilita outras. Use a taxonomia do LeanIX como base conceitual, mas adapte-a ao contexto específico.
6.  **Abrangência**: Cobrir o ciclo de vida da produção, desde o planejamento estratégico até a fabricação e controle de qualidade final, sempre sob a ótica de Manufacturing 4.0.
